...
# Train device: cuda
# real	0m26.407s    !!! Worse with GPU. 
# user	0m27.261s
# sys	0m16.983s

# Macbook Pro:  
# 786.44s user 134.18s system 49% cpu 30:54.03 total
# 13 minutes. 
#
# Slower, sure, but 50X slower?  How? 
# Not as obvious that it's using all the CPUs (i only see one run.py)
# Maybe related: https://github.com/pytorch/pytorch/issues/7686

# RND: 
# real	0m31.138s
# 
# It seems to use all 48 CPUs.  Note m5.12xlarge cost $2.30/hour. 
# We can double the # of CPUs with m5.16xlarge for only $3/hour.
# If that cuts time in 1/2 then it's better deal for this problem. 
...
def run(args):
    ...
    # says 4 with Macbook Pro, but I see no evidence of it in htop. 
    # Could play with torch.set_num_threads(n)
    print(f"Train device: {train.device} Threads: {torch.get_num_threads()}")
